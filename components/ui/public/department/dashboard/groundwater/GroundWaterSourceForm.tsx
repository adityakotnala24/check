...
export interface GroundWaterSourceFormType {
  district: string;
  block: string,
...
const createFormData = (mobileNumber: string): GroundWaterSourceFormType => ({
  district: '',
  block: '',
  gramPanchayat: '',
  revenueVillage: '',
...
const GroundWaterSourceForm = () => {
  const { session } = useSession();
  const sessionInfo = JSON.parse(session || "{}");

  const router = useRouter();
  const [uploding, setUploading] = useState(false);
  const [formData, setFormData] = useState<GroundWaterSourceFormType>(createFormData(sessionInfo.mobileNumber));
  ...
  const validateForm = (): boolean => {
    ...
    const isFormValid = requiredFields.every(
      field => formData[field as keyof GroundWaterSourceFormType]
    );
  ...
  return (
    <View style={styles.container}>
      ...
      {formData.activity === 37 &&
        <CustomInputField
          formData={formData}
          setFormData={setFormData}
          keyName="otherActivity"
...
export default GroundWaterSourceForm;