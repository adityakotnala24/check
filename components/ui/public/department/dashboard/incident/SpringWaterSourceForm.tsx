...
const SpringWaterSourceForm = () => {
  const { session } = useSession();
  const sessionInfo = JSON.parse(session || "{}");

  const router = useRouter();
  const [uploding, setUploading] = useState(false);
  const [formData, setFormData] = useState<SpringWaterSourceFormType>(createFormData(sessionInfo.mobileNumber));
  ...
  const validateForm = (): boolean => {
    ...
    const isFormValid = requiredFields.every(
      field => formData[field as keyof SpringWaterSourceFormType]
    );
...
export default SpringWaterSourceForm;